...
type CartStore = {
  ...
  addVariation: (id: number, variation: string, times?: number) => void;
...
export const useCartStore = create<CartStore>()(
  persist(
    (set, get) => ({
      ...
      addVariation: (id, variation, times) => {
        if (! times) {times = 1}
        const new_var_list = Array<string>(times).fill(variation);
        const newItems: CartItem[] = get().cartItems.map((item) => {
          if (item.id === id) {
            return { 
              ...item, 
              variation_list: [...item.variation_list, ...new_var_list]
            };
          }
          return item;
        })
